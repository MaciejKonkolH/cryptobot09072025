"""
Plik z funkcjami pomocniczymi dla modułu treningowego training4.
...
from training4 import config as trainer_config
...
def save_training_results_to_markdown(evaluation_results, model_params, data_info, cfg):
    ...
    symbol_suffix = f"_{data_info.get('symbol', 'UNKNOWN')}" if data_info.get('symbol') else ""
    filename = f"results{symbol_suffix}_{timestamp}.md"
    
    # Określ katalog wyników na podstawie symbolu
    if data_info.get('symbol'):
        results_dir = trainer_config.get_report_dir(data_info['symbol'])
    else:
        results_dir = Path(__file__).resolve().parent / "output" / "reports"
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        # Nagłówek
        symbol_info = f" - {data_info.get('symbol', 'UNKNOWN')}" if data_info.get('symbol') else ""
        f.write(f"# WYNIKI TRENINGU{symbol_info} - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        
        # Parametry modelu
        f.write("## 📊 PARAMETRY MODELU\n")
        f.write(f"- SYMBOL: {data_info.get('symbol', 'N/A')}\n")
        ...
        future_window = extract_future_window(str(data_info.get('symbol', '')))
