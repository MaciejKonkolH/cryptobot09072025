...
Obsługuje trening dla pojedynczej pary z indywidualnymi scalerami.
"""
...
class MultiOutputXGBoost:
    """
    Klasa do trenowania osobnych modeli XGBoost dla 15 poziomów TP/SL.
    ...
    def __init__(self, symbol=None):
        """
        Inicjalizuje klasę Multi-Output XGBoost.
        
        Args:
            symbol: Symbol pary (np. 'ETHUSDT') dla logowania i ścieżek
        """
        self.symbol = symbol
        self.models = []  # lista 15 modeli XGBoost (native)
    ...
    def train_model(self, X_train, y_train, X_val, y_val, feature_names=None):
        """
        Trenuje modele dla wszystkich poziomów TP/SL.
        
        Args:
            X_train: DataFrame z cechami treningowymi
            y_train: DataFrame z etykietami treningowymi (15 kolumn)
            X_val: DataFrame z cechami walidacyjnymi
            y_val: DataFrame z etykietami walidacyjnymi (15 kolumn)
            feature_names: Lista nazw cech (opcjonalna)
        """
        symbol_info = f"({self.symbol})" if self.symbol else ""
        logger.info(f"Rozpoczynanie treningu osobnych modeli XGBoost {symbol_info} dla każdego poziomu TP/SL...")
        ...
        for i, label_col in enumerate(cfg.LABEL_COLUMNS):
            ...
            logger.info(f"=== TRENING POZIOMU {i+1}/15 {symbol_info} ({level_desc}) ===")
        ...
        logger.info(f"Całkowity czas treningu {symbol_info}: {total_time:.1f}s ({total_time/60:.1f} min)")
        
        # Predykcje walidacyjne
        y_val_pred = self.predict(X_val)
        self._log_validation_metrics(y_val, y_val_pred)
        logger.info(f"Trening wszystkich modeli {symbol_info} zakończony pomyślnie.")
    ...
    def _log_validation_metrics(self, y_val_true, y_val_pred):
        symbol_info = f"({self.symbol})" if self.symbol else ""
        logger.info(f"Metryki walidacyjne {symbol_info}:")
        for i, label_col in enumerate(cfg.LABEL_COLUMNS):
            level_desc = cfg.TP_SL_LEVELS_DESC[i]
