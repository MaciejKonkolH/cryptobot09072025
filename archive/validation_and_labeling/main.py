...
class ValidationAndLabelingPipeline:
    ...
    def process_single_pair(self, pair_name: str, input_file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if config.SAVE_RAW_VALIDATED_DATA:
                ...
                # 🎯 KLUCZOWA POPRAWKA: Poprawnie przypisz strefę czasową UTC, nie konwertuj!
                # Dane są już w UTC, więc tylko to deklarujemy.
            ...
            # == EARLY EXIT: Jeśli LABELING jest wyłączony w konfiguracji ==
            if not getattr(config, "ENABLE_LABELING", True):
                self.logger.info("⚠️  Labeling disabled via config.ENABLE_LABELING = False. Zakończono po walidacji danych.")
                ...
                self.logger.info(f"✅ Validation-only processing finished. Report saved to {report_path}")
            ...
            # KROK 9: Zapisz próbkę etykiet do CSV dla weryfikacji wizualnej
    ...
    def _save_label_sample_to_csv(self, final_df: pd.DataFrame, pair_name: str):
        """Zapisuje próbkę danych (OHLC + cechy + etykieta) do pliku CSV dla weryfikacji wizualnej."""
        
        # Konfiguracja "na twardo", aby uniknąć problemów z plikiem config.py
        SAVE_SAMPLE = True
        SAMPLE_HOURS = 24
        ...
        try:
            self.logger.info(f"Zapisywanie {SAMPLE_HOURS}h próbki etykiet do pliku CSV dla {pair_name}...")
            
            # Stwórz kopię, aby uniknąć modyfikacji oryginalnego DataFrame (SettingWithCopyWarning)
            df_copy = final_df.copy()

            # Dynamicznie znajdź kolumnę z etykietą i przygotuj ją do mapowania
            label_col_name = None
            if 'label' in df_copy.columns:
                label_col_name = 'label'
            elif 'label_1' in df_copy.columns: # Obsługa formatu one-hot
                # SHORT=0, LONG=1, HOLD=2 (zgodnie z competitive_labeler)
                df_copy['temp_label'] = 2  # Domyślnie HOLD
                df_copy.loc[df_copy['label_0'] == 1, 'temp_label'] = 0  # SHORT
                df_copy.loc[df_copy['label_1'] == 1, 'temp_label'] = 1  # LONG
                label_col_name = 'temp_label'

            # Jeśli znaleziono kolumnę z etykietami, zmapuj ją na wartości słowne
            if label_col_name:
                signal_map = {0: 'SHORT', 1: 'LONG', 2: 'HOLD'}
                df_copy['signal'] = df_copy[label_col_name].map(signal_map)
                # Opcjonalnie usuń tymczasową kolumnę, jeśli była tworzona
                if label_col_name == 'temp_label':
                    df_copy.drop(columns=['temp_label'], inplace=True)
            else:
                self.logger.warning("Nie znaleziono kolumny z etykietą ('label' lub 'label_x'). Próbka CSV nie będzie zawierać sygnału.")

            # Upewnijmy się, że DataFrame ma indeks typu Datetime
            if not isinstance(df_copy.index, pd.DatetimeIndex):
                 self.logger.warning("DataFrame nie ma indeksu typu Datetime. Próbuję przekonwertować...")
                 # Spróbuj przekonwertować, jeśli to możliwe, lub zrezygnuj
                 if 'timestamp' in df_copy.columns:
                     df_copy = df_copy.set_index('timestamp')
                 elif 'datetime' in df_copy.columns:
                     df_copy = df_copy.set_index('datetime')
                 else:
                     self.logger.error("Brak kolumny czasowej do ustawienia jako indeks. Nie można zapisać próbki.")
                     return
            ...
            # Sprawdź, które z żądanych kolumn faktycznie istnieją
            ...
            if len(existing_columns) != len(columns_to_save):
                ...
                self.logger.warning(f"Brakujące kolumny w ramce danych: {missing}. Zapisuję tylko dostępne.")
            ...
            # Definicja precyzji zaokrąglenia dla poszczególnych kolumn
            rounding_rules = {
                'open': 4,
                'high': 4,
                'low': 4,
            ...
            # Zaokrąglij wartości w kolumnach, które istnieją w DataFrame
            ...
            self.logger.info(f"✅ Próbka etykiet zapisana pomyślnie do: {full_path}")

        except Exception as e:
            self.logger.error(f"❌ Wystąpił błąd podczas zapisywania próbki etykiet: {e}")
            import traceback
            traceback.print_exc()
